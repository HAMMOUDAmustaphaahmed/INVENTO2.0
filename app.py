...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    'mysql+mysqlconnector://avnadmin:AVNS_C96xtkYSwqY0qMlZ2aS@mysql-2ef51fa8-hammouda-9afc.h.aivencloud.com:19722/invento'
)
...
def admin():
    ...
    for achat in achats_data:
        ...
        achat_prix_totale += achat.prix_achat * achat.quantite
...
def rechercher_article():
        ...
        if request.method == 'POST':
            ...
            if article:
                return render_template('editer_article.html', article=article,articles_data=articles_data)
            else:
...
def editer_article():
        ...
        return render_template('editer_article.html',articles_data=articles_data)  # Rediriger si aucune action trouvée
...
def supprimer_user():
    ...
    if user:
        ...
        message = "User not found."
        return """<script>alert("{message}");window.location.href = "{url_for('editer_article')}";</script>"""
